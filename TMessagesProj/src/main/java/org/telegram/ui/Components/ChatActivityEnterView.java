...
//                    list.add(2);
    ...
    public ChatActivityEnterView(Activity context, SizeNotifierFrameLayout parent, ChatActivity fragment, final boolean isChat, Theme.ResourcesProvider resourcesProvider) {
        ...
        if (isIOSInputStyle()) {
            textFieldContainer.addView(frameLayout, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, LayoutHelper.WRAP_CONTENT, Gravity.BOTTOM, botCommandsMenuButton != null ? 0 : senderSelectView != null ? 5 : 10, 0, 48, 0));
        } else {
        ...
        if (isIOSInputStyle()) {
            ...
            textFieldContainer.addView(aiButton, LayoutHelper.createFrame(33, 33, Gravity.TOP | Gravity.LEFT, 4.66f, 1, 0, 0));
        } else {
            aiButton.setBackground(Theme.createSelectorDrawable(getThemedColor(Theme.key_listSelector), Theme.RIPPLE_MASK_CIRCLE_20DP, dp(16)));
        ...
        if (isIOSInputStyle()) {
            ...
            textFieldContainer.addView(richButton, LayoutHelper.createFrame(33, 33, Gravity.TOP | Gravity.RIGHT, 0, 1, 4.66f, 0));
        } else {
            richButton.setBackground(Theme.createSelectorDrawable(getThemedColor(Theme.key_listSelector), Theme.RIPPLE_MASK_CIRCLE_20DP, dp(16)));
...
//        audioVideoSendButton.setAccessibilityDelegate(mediaMessageButtonsDelegate);
    ...
    private void createBotButton() {
        ...
        if (isIOSInputStyle()) {
            attachLayout.addView(botButton, 0, LayoutHelper.createLinear(DEFAULT_HEIGHT, DEFAULT_HEIGHT, 0, 0, 35, 0));
            updateScheduledButtonMarginsIOS();
        } else {
        ...
        messageEditText = new ChatActivityEditTextCaption(getContext(), resourcesProvider) {
            ...
            protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
                ...
                if (lineCount != messageEditText.getLineCount()) {
                    boolean hasEditorText =
                            messageEditText.getLineCount() > (isIOSInputStyle() ? 3 : 2)
                                    && messageEditText.getText() != null
                                    && !TextUtils.isEmpty(
...
if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.VANILLA_ICE_CREAM) {
    ...
    private void showAiButton(boolean show_) {
        final boolean show =
                (show_ || richDraftActive)
                        && parentFragment != null
                        && !parentFragment.isSecretChat()
                        && !NaConfig.INSTANCE.getHideAiEditor().Bool();
...
//        }
    ...
    protected boolean sendMessageInternal(boolean notify, int scheduleDate, int scheduleRepeatPeriod, long payStars, boolean allowConfirm) {
        return sendMessageInternal(
                ...
                payStars,
                allowConfirm,
                new SendMessageInternalParams()
        );
    }

    protected boolean sendMessageInternal(
            boolean notify,
            int scheduleDate,
            int scheduleRepeatPeriod,
            long payStars,
            boolean allowConfirm,
            SendMessageInternalParams internalParams
    ) {
        final boolean allowConfirmFinal =
                allowConfirm
        ...
        final Runnable send = () -> {
            ...
            if (!TextUtils.isEmpty(message)) {
                ...
                if (processSendingText(message, notify, scheduleDate, scheduleRepeatPeriod, payStars, internalParams)) {
                    // The message is already enqueued successfully. Clear the composer and
                    // reply state atomically instead of waiting for the send animation callback;
                    // the delayed callback kept the old reply visible and allowed duplicate taps.
                    messageTransitionIsRunning = false;
        ...
        if (allowConfirmFinal) {
            boolean alertShown =
                    AlertsCreator.ensurePaidMessageConfirmation(
                            currentAccount,
                            dialog_id,
                            getMessagesCount(),
                            starsPrice -> sendMessageInternal(
                                    notify,
                                    scheduleDate,
                                    scheduleRepeatPeriod,
                                    starsPrice,
...
//            }
    ...
    private void updateScheduledButtonMarginsIOS() {
        ...
        ((LinearLayout.LayoutParams) lp).rightMargin = botButtonDrawable == null ? dp(35) : 0;
