...
import toss.authorgram.filters.popup.RegexFilterPopup;
import toss.authorgram.settings.BaseAGSettingsActivity;
import tw.nekomimi.nekogram.ui.cells.HeaderCell;

public class RegexSharedFiltersListActivity extends BaseAGSettingsActivity {
    ...
    private ArrayList<AGFilter.FilterModel> getDisplayedFilters() {
        ArrayList<AGFilter.FilterModel> filters = new ArrayList<>(AGFilter.getRegexFilters());
        if (exclusionDialogId != 0L) {
            filters.removeIf(filter -> filter == null || AGFilter.isSharedFilterExcluded(exclusionDialogId, filter.id));
        }
        return filters;
    }
    ...
    protected void updateRows() {
        ...
        ArrayList<AGFilter.FilterModel> filters = getDisplayedFilters();
    ...
    protected void onItemClick(View view, int position, float x, float y) {
        if (position >= startRow && position < endRow) {
            ...
            ArrayList<AGFilter.FilterModel> filterModels = getDisplayedFilters();
            if (filterIndex < 0 || filterIndex >= filterModels.size()) {
                return;
            }
            if (exclusionDialogId != 0L) {
                AGFilter.FilterModel filterModel = filterModels.get(filterIndex);
                if (filterModel != null) {
                    AGFilter.setSharedFilterExcluded(exclusionDialogId, filterModel.id, true);
                }
                finishFragment();
                return;
            }
                ...
                case TYPE_TEXT:
                    if (position >= startRow && position < endRow) {
                        ...
                        if (idx >= 0 && idx < filters.size()) {
                            ...
                            AGFilter.FilterModel model = filters.get(idx);
                            boolean divider = position + 1 < endRow;
                            if (exclusionDialogId != 0L) {
                                textCell.setColors(-1, Theme.key_windowBackgroundWhiteBlackText);
                                textCell.setText(AGFilter.getFilterDisplayText(model), divider);
                            } else {
