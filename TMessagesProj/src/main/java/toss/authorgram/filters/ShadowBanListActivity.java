...
import toss.authorgram.filters.popup.RegexUserFilterPopup;
import toss.authorgram.settings.BaseAGSettingsActivity;
import tw.nekomimi.nekogram.ui.cells.FiltersChatCell;

public class ShadowBanListActivity extends BaseAGSettingsActivity {
    ...
    private ArrayList<Long> buildShadowBannedPeers() {
        ...
        merged.addAll(AGFilter.getBlockedChannelsList());
        merged.addAll(AGFilter.getCustomFilteredUsersList());
        return merged;
    }
    ...
    private void addShadowBannedPeer(long dialogId) {
        ...
        HashSet<Long> idSet = new HashSet<>(AGFilter.getCustomFilteredUsersList());
        if (!idSet.add(dialogId)) {
            return;
        }
        AGFilter.setCustomFilteredUsers(new ArrayList<>(idSet));
        TLRPC.User localUser = getMessagesController().getUser(dialogId);
        if (localUser != null) {
            AGFilter.updateCustomFilteredUserFromLocalUser(localUser);
        }
        refreshRows();
    }
    ...
    private boolean isCustomFilteredUserUntracked(long userId) {
        return userId <= 0L || !AGFilter.getCustomFilteredUsersList().contains(userId);
    }
    ...
    private String buildFallbackCustomFilteredUserTitle(AGFilter.CustomFilteredUser userData, long userId) {
        if (userData != null) {
    ...
    private String getCustomFilteredUserRowTitle(long userId) {
        ...
        AGFilter.CustomFilteredUser userData = AGFilter.getCustomFilteredUser(userId);
    ...
    private void resolveCustomFilteredUserFromLocalDb(long userId) {
        Utilities.globalQueue.postRunnable(() -> {
            ...
            AndroidUtilities.runOnUIThread(() -> {
                ...
                if (storageUser != null) {
                    getMessagesController().putUser(storageUser, true);
                    AGFilter.updateCustomFilteredUserFromLocalUser(storageUser);
                }
