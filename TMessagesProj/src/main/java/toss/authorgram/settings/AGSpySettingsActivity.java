...
public class AGSpySettingsActivity extends BaseAGXSettingsActivity {
    ...
    private final AbstractConfigCell headerSpy =
            cellGroup.appendCell(new ConfigCellHeader(getString(R.string.AuthorGramSpy)));
    private final AbstractConfigCell ghostModeRow =
            cellGroup.appendCell(new ConfigCellTextCheckIcon(
                    null,
                    "GhostMode",
                    null,
    ...
    private final AbstractConfigCell regexFiltersRow =
            cellGroup.appendCell(new ConfigCellTextCheckIcon(
                    null,
                    "RegexFilters",
                    null,
    ...
    private final AbstractConfigCell spySettingsRow =
            cellGroup.appendCell(new ConfigCellTextCheckIcon(
                    null,
                    "AyuSpySettings",
                    null,
                    R.drawable.msg_bots_solar,
                    true,
                    () -> presentFragment(new AGPrivacySettingsActivity())
            ));
    private final AbstractConfigCell customHeaderRow =
            cellGroup.appendCell(new ConfigCellHeader(getString(R.string.AyuMomentsCustomHeader)));
    private final AbstractConfigCell deletedMessagesPreviewRow =
            cellGroup.appendCell(new ConfigCellCustom(
                    "DeletedMessagesAppearancePreviewRow",
                    ConfigCellCustom.CUSTOM_ITEM_DeletedMessagesAppearanceCard,
                    false
            ));
    private final AbstractConfigCell translucentDeletedMessagesRow =
            cellGroup.appendCell(new ConfigCellTextCheck(
                    NaConfig.INSTANCE.getTranslucentDeletedMessages()
            ));
    private final AbstractConfigCell deletedMarkRow =
            cellGroup.appendCell(new ConfigCellCustom(
                    NaConfig.INSTANCE.getDeletedIconStyle().getKey(),
                    CellGroup.ITEM_TYPE_TEXT_SETTINGS_CELL,
                    true
            ));
    private final AbstractConfigCell deletedMarkColorRow =
            cellGroup.appendCell(new ConfigCellCustom(
                    NaConfig.INSTANCE.getDeletedIconColor().getKey(),
                    ConfigCellCustom.CUSTOM_ITEM_DeletedMessagesColorPicker,
                    false
            ));
    private final AbstractConfigCell customDeletedMarkRow =
            cellGroup.appendCell(new ConfigCellTextInput(
                    null,
                    NaConfig.INSTANCE.getCustomDeletedMark(),
                    "",
                    null
            ));
    private final AbstractConfigCell dividerPrivateFeatures =
            cellGroup.appendCell(new ConfigCellDivider());
    private final AbstractConfigCell localPremiumRow =
            cellGroup.appendCell(new ConfigCellTextCheck(NekoConfig.localPremium));
    private final AbstractConfigCell hideProxySponsorChannelRow =
            cellGroup.appendCell(new ConfigCellTextCheck(NekoConfig.hideProxySponsorChannel));
    private final AbstractConfigCell hideSponsoredMessageRow =
            cellGroup.appendCell(new ConfigCellTextCheck(NekoConfig.hideSponsoredMessage));
    private final AbstractConfigCell dividerPrivateFeaturesBottom =
            cellGroup.appendCell(new ConfigCellDivider());

    public AGSpySettingsActivity() {
    ...
    public String getTitle() {
        return getString(R.string.AuthorGramSpy);
    }
    ...
    public ArrayList<ThemeDescription> getThemeDescriptions() {
        ...
        themeDescriptions.add(new ThemeDescription(
                listView,
                ThemeDescription.FLAG_CELLBACKGROUNDCOLOR,
        ...
        if (context != null && value != null) {
            ...
            new AlertDialog.Builder(context)
                    ...
                    .setNegativeButton(
                            ...
                            (dialogInter, i) -> scrollToRow(key, unknown)
                    )
    ...
    public void scrollToRow(String key, Runnable unknown) {
        if (NaConfig.INSTANCE.getDeletedIconColor().getKey().equals(key)
                && !cellGroup.rows.contains(deletedMarkColorRow)) {
        ...
        if (NaConfig.INSTANCE.getCustomDeletedMark().getKey().equals(key)
                && !cellGroup.rows.contains(customDeletedMarkRow)) {
            String rowKey = getRowKey(deletedMarkRow);
            super.scrollToRow(rowKey != null ? rowKey : key, unknown);
            return;
        }
    ...
    private void checkDeletedMarkDetailRows() {
        ...
        AbstractConfigCell rowToShow =
                showColorRow ? deletedMarkColorRow : customDeletedMarkRow;
        AbstractConfigCell rowToHide =
                showColorRow ? customDeletedMarkRow : deletedMarkColorRow;
    ...
    private void bindDeletedMarkRow(TextSettingsCell cell) {
        cell.setTextAndValue(
                getString(R.string.DeletedMarkText),
                null,
                cellGroup.needSetDivider(deletedMarkRow)
        );
    ...
    private void showDeletedMarkDialog() {
        ...
        for (int i = 0; i < entries.length; i++) {
            RadioColorCell cell =
                    new RadioColorCell(context, getResourceProvider());
            cell.setPadding(
    ...
    private class ListAdapter extends BaseListAdapter {
        ...
        @Override
        protected View onCreateCustomViewHolder(
                @NonNull ViewGroup parent,
                int viewType
        ) {
            if (viewType
                    == ConfigCellCustom.CUSTOM_ITEM_DeletedMessagesAppearanceCard) {
                deletedMessagesPreviewCell =
                        new DeletedMessagesPreviewCell(
                                mContext,
                                getParentLayout()
                        );
                return deletedMessagesPreviewCell;
            } else if (viewType
                    == ConfigCellCustom.CUSTOM_ITEM_DeletedMessagesColorPicker) {
                deletedMessagesColorPickerCell =
                        new DeletedMessagesColorPickerCell(
                                mContext,
                                getResourceProvider()
                        );
        ...
        @Override
        protected void onBindCustomViewHolder(
            ...
            if (row == deletedMessagesPreviewRow) {
                DeletedMessagesPreviewCell previewCell =
                        (DeletedMessagesPreviewCell) holder.itemView;
                previewCell.refresh();
            } else if (row == deletedMarkRow) {
                bindDeletedMarkRow((TextSettingsCell) holder.itemView);
            } else if (row == deletedMarkColorRow) {
                DeletedMessagesColorPickerCell colorPickerCell =
                        (DeletedMessagesColorPickerCell) holder.itemView;
                colorPickerCell.setSelectedColorId(
                        NaConfig.INSTANCE.getDeletedIconColor().Int(),
                        false
                );
            }
        }
