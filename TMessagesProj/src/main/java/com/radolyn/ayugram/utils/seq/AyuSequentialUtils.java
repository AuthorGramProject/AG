...
public abstract class AyuSequentialUtils {
    ...
    public static boolean loadDocumentsSync(int currentAccount, ArrayList<MessageObject> messages) {
        ...
        for (int attempt = 0; attempt < maxAttempts; attempt++) {
            ...
            for (int i = 0; i < messages.size(); i++) {
                ...
                if (!needsLocalCopy(messageObject) || hasLocalCopy(currentAccount, messageObject) || messageObject.isAyuDeleted()) {
                    continue;
                }
            ...
            if (allLocalCopiesReady(currentAccount, messages)) {
                return true;
            }
            if (!waiter.hasFailed()) {
                break;
    ...
    private static boolean hasLocalCopy(int currentAccount, MessageObject messageObject) {
        ...
        String path = FileLoader.getInstance(currentAccount).getPathToMessage(messageObject.messageOwner).toString();
    ...
    private static boolean allLocalCopiesReady(int currentAccount, ArrayList<MessageObject> messages) {
        ...
        for (int i = 0; i < messages.size(); i++) {
            MessageObject messageObject = messages.get(i);
            if (needsLocalCopy(messageObject) && !messageObject.isAyuDeleted() && !hasLocalCopy(currentAccount, messageObject)) {
                return false;
            }
        }
        return true;
