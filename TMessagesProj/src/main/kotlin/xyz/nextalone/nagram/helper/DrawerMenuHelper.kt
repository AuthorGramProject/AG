...
object DrawerMenuHelper {
    ...
    @JvmStatic
    private val allEntries: List<Entry> = listOf(
        Entry(DrawerLayoutAdapter.agbtnGhostMode, R.string.GhostMode, R.drawable.ayu_ghost),
        ...
        Entry(DrawerLayoutAdapter.agbtnArchivedChats, R.string.ArchivedChats, R.drawable.msg_archive),
        ...
        Entry(DrawerLayoutAdapter.agbtnRecentChats, R.string.RecentChats, R.drawable.msg_recent),
        Entry(ID_SAVED, R.string.SavedMessages, R.drawable.msg_saved),
        Entry(DrawerLayoutAdapter.agbtnBookmarks, R.string.BookmarksManager, R.drawable.msg_fave),
        Entry(ID_SETTINGS, R.string.Settings, R.drawable.msg_settings_old),
        Entry(DrawerLayoutAdapter.agbtnSettings, R.string.AGSettings, R.drawable.ag_settings),
        Entry(DrawerLayoutAdapter.agbtnBrowser, R.string.InappBrowser, R.drawable.web_browser),
        Entry(DrawerLayoutAdapter.agbtnQrLogin, R.string.ImportLogin, R.drawable.msg_qrcode),
        Entry(DrawerLayoutAdapter.agbtnSessions, R.string.Devices, R.drawable.msg2_devices),
        Entry(DrawerLayoutAdapter.agbtnMainTabsCustomize, R.string.MainTabsCustomize, R.drawable.tabs_reorder),
        Entry(DrawerLayoutAdapter.agbtnRestartApp, R.string.RestartApp, R.drawable.msg_retry)
    )

    /** Play never exposes rows disabled by the AuthorGram policy boundary. */
    @JvmStatic
    val entries: List<Entry>
        get() = if (AuthorGramPlayPolicy.isPlayBuild()) {
            allEntries.filterNot { it.id == DrawerLayoutAdapter.agbtnGhostMode }
        } else {
            allEntries
        }
    ...
    @JvmStatic
    fun defaultLayout(): MutableList<Int> = mutableListOf<Int>().apply {
        if (!AuthorGramPlayPolicy.isPlayBuild()) {
            add(DrawerLayoutAdapter.agbtnGhostMode)
            add(DIVIDER)
        }
        addAll(listOf(
            ID_MY_PROFILE, ID_EMOJI_STATUS, ID_NEW_GROUP, ID_CONTACTS, ID_CALLS,
            DrawerLayoutAdapter.agbtnRecentChats, ID_SAVED, ID_SETTINGS, DIVIDER,
            DrawerLayoutAdapter.agbtnSettings
        ))
    }

    @JvmStatic
    fun defaultHidden(): MutableList<Int> = mutableListOf(
        DrawerLayoutAdapter.agbtnArchivedChats, ID_NEW_CHANNEL,
        DrawerLayoutAdapter.agbtnBookmarks, DrawerLayoutAdapter.agbtnBrowser,
        DrawerLayoutAdapter.agbtnQrLogin, DrawerLayoutAdapter.agbtnSessions,
        DrawerLayoutAdapter.agbtnMainTabsCustomize, DrawerLayoutAdapter.agbtnRestartApp
    )
    ...
    private fun sanitize(layout: MutableList<Int>, hidden: MutableList<Int>): Boolean {
        ...
        val hiddenIterator = hidden.iterator()
        while (hiddenIterator.hasNext()) {
            if (entryFor(hiddenIterator.next()) == null) {
                hiddenIterator.remove()
                changed = true
            }
        }
        for (index in layout.indices.reversed()) {
            if (layout[index] == DIVIDER &&
                (index == 0 || index == layout.lastIndex || layout[index - 1] == DIVIDER)
            ) {
    ...
    private fun migrateFromLegacyToggles() {
        ...
        if (!AuthorGramPlayPolicy.isPlayBuild()) {
            put(DrawerLayoutAdapter.agbtnGhostMode, NekoConfig.showGhostInDrawer.Bool())
        }
        put(ID_MY_PROFILE, NaConfig.drawerItemMyProfile.Bool())
        put(ID_EMOJI_STATUS, NaConfig.drawerItemSetEmojiStatus.Bool())
        put(DrawerLayoutAdapter.agbtnArchivedChats, NaConfig.drawerItemArchivedChats.Bool())
        ...
        put(DrawerLayoutAdapter.agbtnRecentChats, NaConfig.drawerItemRecentChats.Bool())
        put(ID_SAVED, NaConfig.drawerItemSaved.Bool())
        put(DrawerLayoutAdapter.agbtnBookmarks, NaConfig.showAddToBookmark.Bool())
        put(ID_SETTINGS, NaConfig.drawerItemSettings.Bool())
        put(DrawerLayoutAdapter.agbtnSettings, NaConfig.drawerItemNSettings.Bool())
        put(DrawerLayoutAdapter.agbtnBrowser, NaConfig.drawerItemBrowser.Bool())
        put(DrawerLayoutAdapter.agbtnQrLogin, NaConfig.drawerItemQrLogin.Bool())
        put(DrawerLayoutAdapter.agbtnSessions, NaConfig.drawerItemSessions.Bool())
        put(DrawerLayoutAdapter.agbtnRestartApp, NaConfig.drawerItemRestartApp.Bool())

        // Introduced after the legacy toggles; default to hidden, the manager can enable it.
        if (!hidden.contains(DrawerLayoutAdapter.agbtnMainTabsCustomize)) {
            hidden.add(DrawerLayoutAdapter.agbtnMainTabsCustomize)
        }
        ...
        val ghostIndex = visible.indexOf(DrawerLayoutAdapter.agbtnGhostMode)
        if (ghostIndex >= 0) visible.add(ghostIndex + 1, DIVIDER)
        val nSettingsIndex = visible.indexOf(DrawerLayoutAdapter.agbtnSettings)
