...
object MainMenuActions {
    ...
    fun isSupported(id: Int): Boolean {
        return id != DrawerMenuHelper.ID_EMOJI_STATUS &&
                id != DrawerLayoutAdapter.agbtnQrLogin &&
                !(AuthorGramPlayPolicy.isPlayBuild() && id == DrawerLayoutAdapter.agbtnGhostMode) &&
    ...
    fun openItem(id: Int, fragment: BaseFragment, currentAccount: Int): Boolean {
        when {
            id == DrawerLayoutAdapter.agbtnGhostMode && !AuthorGramPlayPolicy.isPlayBuild() -> toggleGhostMode(fragment)
            id == DrawerLayoutAdapter.agbtnRecentChats ->
            ...
            id == DrawerLayoutAdapter.agbtnArchivedChats -> {
                val args = Bundle()
                args.putInt("folderId", 1)
                fragment.presentFragment(DialogsActivity(args))
            }
            id == DrawerLayoutAdapter.agbtnBookmarks ->
                fragment.presentFragment(BookmarkManagerActivity())
            id == DrawerLayoutAdapter.agbtnSettings ->
                fragment.presentFragment(AGSettingsActivity())
            id == DrawerLayoutAdapter.agbtnBrowser -> BrowserUtils.openBrowserHome(null, true)
            id == DrawerLayoutAdapter.agbtnSessions ->
                fragment.presentFragment(SessionsActivity(SessionsActivity.TYPE_DEVICES))
            id == DrawerLayoutAdapter.agbtnMainTabsCustomize ->
                fragment.presentFragment(MainTabsCustomizeActivity())
            id == DrawerLayoutAdapter.agbtnRestartApp -> AppRestartHelper.triggerRebirth(
    ...
    fun longClickItem(id: Int, fragment: BaseFragment, currentAccount: Int): Boolean {
        when {
            id == DrawerLayoutAdapter.agbtnGhostMode && !AuthorGramPlayPolicy.isPlayBuild() ->
                fragment.presentFragment(GhostModeActivity())
            id == DrawerLayoutAdapter.agbtnBrowser ->
