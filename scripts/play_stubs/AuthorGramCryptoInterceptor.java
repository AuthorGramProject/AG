...
/**
 * Play build: incoming AuthorGram payload compatibility is retained, while
 * outgoing custom-wire encryption is intentionally absent.
 */
public final class AuthorGramCryptoInterceptor {
    private AuthorGramCryptoInterceptor() { }

    public static boolean prepareOutgoingRequest(int account, TLObject request, MessageObject messageObject) {
    ...
    public static boolean decryptIncomingMessage(int account, TLRPC.Message message) {
        if (message == null || message.message == null || !AuthorGramCrypto.isAuthorGramPayload(message.message)) {
            return false;
        }
        String plaintext = AuthorGramChatCrypto.decryptTextOrNull(account, MessageObject.getDialogId(message), message.message);
        if (plaintext == null) return false;
        message.message = plaintext;
        if (message.entities != null) message.entities.clear();
        AuthorGramMessageMeta.markDecrypted(account, message);
        return true;
    }

    private static void sanitizeReplyToEncryptedSource(int account, TLObject request, MessageObject outgoingMessage) {
        MessageObject repliedMessage = outgoingMessage.replyMessageObject;
        if (!AuthorGramMessageMeta.isKnownEncrypted(account, repliedMessage)) return;
        try {
            Field replyField = request.getClass().getField("reply_to");
            Object replyValue = replyField.get(request);
            if (replyValue instanceof TLRPC.InputReplyTo) sanitizeEncryptedReply((TLRPC.InputReplyTo) replyValue);
    ...
    private static void sanitizeEncryptedReply(TLRPC.InputReplyTo replyTo) {
        if (!(replyTo instanceof TLRPC.TL_inputReplyToMessage)) return;
        replyTo.flags &= ~(TLObject.FLAG_2 | TLObject.FLAG_3 | TLObject.FLAG_4);
        replyTo.quote_text = null;
        if (replyTo.quote_entities != null) replyTo.quote_entities.clear();
        replyTo.quote_offset = 0;
    }

    private static void sanitizeLocalReplyHeader(MessageObject outgoingMessage) {
        if (outgoingMessage.messageOwner == null || outgoingMessage.messageOwner.reply_to == null) return;
        ...
        if (entities instanceof List) ((List<?>) entities).clear();
        Object flags = getFieldValue(header, "flags");
        if (flags instanceof Integer) setFieldValue(header, "flags", ((Integer) flags) & ~(64 | 128 | 1024));
    }
